...
export function compareStructure(
  ...
  const missingCount = missingTexts.length + missingLinks.length + missingImages.length;
  const score = totalItems > 0 ? Math.round(((totalItems - missingCount) / totalItems) * 100) : 100;
 ...
 */
export function checkConformance(targetHtml: string, outputHtml: string): ConformanceResult {
