...
export function measureMemoryOverhead(fn: () => void): {
  ...
  return {
    before: { heapUsedMB: toMB(before.heapUsed), rssMB: toMB(before.rss), externalMB: toMB(before.external) },
    after: { heapUsedMB: toMB(after.heapUsed), rssMB: toMB(after.rss), externalMB: toMB(after.external) },
    delta: {
